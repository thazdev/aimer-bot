import { ActionRowBuilder, ButtonBuilder, ButtonStyle, TextChannel, Client, ModalBuilder, TextInputBuilder, TextInputStyle, ButtonInteraction, ModalSubmitInteraction, StringSelectMenuBuilder } from "discord.js";
...
export async function setupVerificationButton(client: Client) {
  ...
  const alreadySent = messages.some((msg) =>
    msg.components[0]?.components.some((component) => component.customId === "openVerificationModal"));
  if (alreadySent) {
    return;
  }
  ...
  const row = new ActionRowBuilder<ButtonBuilder>().addComponents(sendVerificationButton);
  await channel.send({
...
export async function handleOpenVerificationModal(interaction: ButtonInteraction) {
  ...
  const row1 = new ActionRowBuilder<TextInputBuilder>().addComponents(agentesInput);
  const row2 = new ActionRowBuilder<TextInputBuilder>().addComponents(focusInput);
  const row3 = new ActionRowBuilder<TextInputBuilder>().addComponents(peripheralsInput);
  const row4 = new ActionRowBuilder<TextInputBuilder>().addComponents(vodreviewInput);
...
export async function handleSubmitVerificationModal(interaction: ModalSubmitInteraction, client: Client) {
